...
def step(context,gene,cortege):
        assert_that(context.solution.calc_count_of_repetitions(gene),equal_to(toIntArray(cortege.split(','))))


@then('Cortege of excesses letters from "{gene}" is "{cortege}"')
def step(context,gene,cortege):
        assert_that(context.solution.get_excess_elements(toIntArray(gene.split(','))),equal_to(toIntArray(cortege.split(','))))
